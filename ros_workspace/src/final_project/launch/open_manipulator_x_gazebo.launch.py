...
def generate_launch_description():
    ...
    custom_world_path = os.path.join(final_project_path, 'world', 'final_world.sdf')
    ...
    gazebo_resource_path = SetEnvironmentVariable(
        ...
        value=[
            ...
            ':' + os.path.join(final_project_path, 'models'),
        ],
    )

    arguments = LaunchDescription([
        DeclareLaunchArgument(
            'world', default_value=custom_world_path, description='Gz sim World'
        ),
    ])
    ...
    # 큐브 생성 파이썬 스크립트 실행 노드 추가
    spawn_cubes_node = Node(
        package='final_project',
        executable='spawn_cube',
        output='screen',
    )

    # 아루코 마커 인식 파이썬 스크립트 실행 노드 추가
